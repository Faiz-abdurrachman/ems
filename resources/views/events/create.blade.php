...
<div class="mx-auto max-w-2xl">
    ...
    <div class="border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000]">
        <div class="border-b border-black px-6 py-5">
            ...
            <p class="mt-1 text-sm text-ink/70 font-bold">Fill in the details below to create a new event.</p>
        </div>
        ...
        <form action="{{ route('admin.events.store') }}" method="POST" class="px-6 py-5 space-y-5" enctype="multipart/form-data">
            ...
            <div>
                <label for="image" class="block text-sm font-bold text-ink">Poster Event <span class="text-ink/60 font-normal">(opsional)</span></label>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="ongoing" {{ old('status') == 'ongoing' ? 'selected' : '' }}>Berlangcyang</option>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-black pt-5">
                <a href="{{ route('admin.events.index') }}" class="inline-flex items-center justify-center rounded-none border border-black bg-white px-4 py-2.5 text-sm font-bold text-ink shadow-[4px_4px_0px_0px_#000] hover:bg-cyan/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors">
                    Cancel
                </a>
