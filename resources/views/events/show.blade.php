    ...
    <div class="overflow-hidden border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000]">
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <div class="mt-1 flex items-center gap-2">
                        ...
                        @endif
                        <span class="text-sm text-ink/50 font-bold">
        ...
        <div class="px-6 py-5">
            <dl class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                <div>
                    <dt class="text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Event Date</dt>
                    <dd class="mt-1 text-sm text-ink">{{ \Carbon\Carbon::parse($event->event_date)->format('d M Y, H:i') }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Location</dt>
                    <dd class="mt-1 text-sm text-ink">{{ $event->location }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Kuota</dt>
                    <dd class="mt-1 text-sm text-ink">{{ $event->quota }} peserta</dd>
                </div>
                <div>
                    <dt class="text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Jumlah Registrasi</dt>
                    <dd class="mt-1 text-sm font-extrabold text-ink">{{ $event->registrations_count ?? 0 }} / {{ $event->quota }}</dd>
                </div>
                @if ($event->description)
                    <div class="sm:col-span-2">
                        <dt class="text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Deskripsi</dt>
        ...
        <div class="overflow-x-auto">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-black">
                            <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Name</th>
                            <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Email</th>
                            <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Phone</th>
                            <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-ink/50 font-bold">Registration Date</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y-2 divide-black">
                        @foreach ($event->registrations as $reg)
                            <tr class="hover:bg-sun/10 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-6 py-3 text-sm text-ink/50 font-bold">{{ $reg->registration_date->format('d M Y, H:i') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @else
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p class="mt-3 text-sm font-bold text-ink/50 font-bold">No participants registered yet</p>
                    <p class="mt-1 text-xs text-ink/40">Participants will appear here once they register</p>
                </div>
            @endif
        </div>
    </div>
