...
<div class="space-y-8">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="relative border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000] hover:scale-[1.02] hover:shadow-[3px_3px_0px_0px_#000] transition-all cursor-default sticker">
            <div class="absolute -top-3 -right-3 text-lg font-black text-sun">✦</div>
            <div class="absolute top-0 left-0 w-full h-2 bg-sun"></div>
            <div class="p-6 pt-7">
                ...
                <p class="mt-3 text-xs font-extrabold uppercase tracking-widest text-ink/40">Total Events</p>
        ...
        <div class="relative border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000] hover:scale-[1.02] hover:shadow-[3px_3px_0px_0px_#000] transition-all cursor-default">
            ...
            <div class="p-6 pt-7">
                ...
                <p class="mt-3 text-xs font-extrabold uppercase tracking-widest text-ink/40">Total Participants</p>
        ...
        <div class="relative border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000] hover:scale-[1.02] hover:shadow-[3px_3px_0px_0px_#000] transition-all cursor-default">
            ...
            <div class="p-6 pt-7">
                ...
                <p class="mt-3 text-xs font-extrabold uppercase tracking-widest text-ink/40">Total Registrations</p>
        ...
        <div class="relative border-2 border-black bg-white shadow-[6px_6px_0px_0px_#000] hover:scale-[1.02] hover:shadow-[3px_3px_0px_0px_#000] transition-all cursor-default">
            ...
            <div class="p-6 pt-7">
                ...
                <p class="mt-3 text-xs font-extrabold uppercase tracking-widest text-ink/40">Active Events</p>
            ...
            <div class="overflow-x-auto">
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            @foreach ($recentRegistrations as $i => $registration)
                                <tr class="border-b border-black {{ $i % 2 == 0 ? 'bg-white' : 'bg-sun/10' }} hover:bg-sun/30 transition-colors">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-3 text-sm font-bold text-ink/50">{{ $registration->created_at->format('d M Y') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                @else
                    <div class="flex flex-col items-center justify-center py-14 text-center">
                        ...
                        <p class="mt-3 text-sm font-extrabold text-ink/30 uppercase">Belum ada data</p>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div>
                @if (isset($upcomingEvents) && count($upcomingEvents) > 0)
                    @foreach ($upcomingEvents as $event)
                        <div class="px-6 py-4 border-b border-black hover:bg-sun/10 transition-colors">
                            <a href="{{ route('admin.events.show', $event) }}" class="block">
                                ...
                                <div class="mt-2 h-2 border border-black bg-gray-100">
                                    <div class="h-full bg-sun" style="width: {{ $event->quota > 0 ? min(($event->registrations_count ?? 0) / $event->quota * 100, 100) : 0 }}%"></div>
                                </div>
                            </a>
                        </div>
                    @endforeach
                @else
                    <div class="flex flex-col items-center justify-center py-14 text-center">
                        <p class="text-sm font-extrabold text-ink/30 uppercase">Belum ada event</p>
                    </div>
                @endif
            </div>
        </div>
