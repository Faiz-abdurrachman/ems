    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="flex flex-wrap gap-3">
                <span class="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-1.5 text-sm font-bold text-blue-700">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    {{ \Carbon\Carbon::parse($event->event_date)->format('d M Y, H:i') }}
                </span>
                <span class="inline-flex items-center gap-1.5 rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-bold text-ink">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    {{ $event->location }}
                </span>
                <span class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold {{ $event->registrations_count >= $event->quota ? 'bg-coral/20 text-ink' : 'bg-mint/20 text-emerald-700' }}">
        ...
        <div>
            <div class="sticky top-24 rounded-xl border border-black bg-white p-6 shadow-sm">
                <h2 class="text-lg font-extrabold text-ink">Daftar Event Ini</h2>

                @if(now()->gt($event->event_date))
                    <div class="mt-4 rounded-lg bg-gray-50 p-4 text-center text-sm text-ink/50 font-bold">Event telah berakhir</div>
                @elseif($event->registrations_count >= $event->quota)
                    <div class="mt-4 rounded-lg bg-coral/20 p-4 text-center text-sm font-bold text-ink">Kuota sudah penuh</div>
                @else
                    <form method="POST" action="{{ route('events.register', $event) }}" class="mt-4 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                class="mt-1 block w-full rounded-lg border-black shadow-sm focus:border-black focus:ring-0 sm:text-sm {{ $errors->has('name') ? 'border-red-300' : '' }}"
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email') }}" required
                                class="mt-1 block w-full rounded-lg border-black shadow-sm focus:border-black focus:ring-0 sm:text-sm {{ $errors->has('email') ? 'border-red-300' : '' }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}"
                                class="mt-1 block w-full rounded-lg border-black shadow-sm focus:border-black focus:ring-0 sm:text-sm {{ $errors->has('phone') ? 'border-red-300' : '' }}"
                                placeholder="08xxxxxxxxxx">
                            @error('phone')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                        </div>
                        @error('event')<p class="text-sm text-red-600 font-bold">{{ $message }}</p>@enderror
                        <button type="submit" class="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-extrabold text-white shadow-sm hover:bg-black hover:text-white transition-colors">Daftar Sekarang</button>
                    </form>
                @endif
