...
<div class="mx-auto max-w-7xl px-6">
    <div class="text-center mb-10">
        <h1 class="text-3xl font-bold tracking-tight text-ink sm:text-4xl">Event Kampus</h1>
        ...
        <form method="GET" action="{{ route('home') }}" class="mt-6 mx-auto max-w-md relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari event..."
                class="block w-full rounded-none border-black pl-10 pr-4 py-2.5 text-sm shadow-[4px_4px_0px_0px_#000] focus:bg-cyan/20 focus:outline-none">
            @if(request('search'))
    ...
    @if(isset($categories) && $categories->isNotEmpty())
        <div class="mx-auto max-w-7xl px-6 -mt-4 mb-6">
            <div class="flex flex-wrap justify-center gap-2">
                <a href="{{ route('home') }}" class="rounded-none px-4 py-1.5 text-xs font-bold {{ !request('category') ? 'bg-cyan text-ink' : 'bg-gray-100 text-ink/70 hover:bg-gray-200' }} transition-colors">Semua</a>
                @foreach($categories as $cat)
                    <a href="{{ route('home', ['category' => $cat->id]) }}" class="rounded-none px-4 py-1.5 text-xs font-bold {{ request('category') == $cat->id ? 'bg-cyan text-ink' : 'bg-gray-100 text-ink/70 hover:bg-gray-200' }} transition-colors">{{ $cat->name }}</a>
                @endforeach
            </div>
        </div>
    @endif
                    ...
                    <div class="p-5">
                        ...
                        <div class="mt-3 flex items-center gap-2">
                            <div class="h-1.5 flex-1 overflow-hidden rounded-none bg-gray-100">
                                <div class="h-full rounded-none {{ ($event->registrations_count >= $event->quota) ? 'bg-coral/200' : 'bg-cyan' }} transition-all" style="width: {{ $event->quota > 0 ? min($event->registrations_count / $event->quota * 100, 100) : 0 }}%"></div>
                            </div>
