<aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-20 flex w-64 flex-col bg-ink bg-stripes text-white border-r-2 border-black transition-transform lg:translate-x-0 -translate-x-full">
    <div class="flex h-14 items-center gap-3 px-5 border-b-2 border-white/20">
        <div class="sticker flex h-9 w-9 items-center justify-center border-2 border-sun bg-sun">
    ...
    <nav class="flex-1 space-y-1 px-3 py-4">
        <a href="{{ route('admin.dashboard') }}"
           class="flex items-center gap-3 px-3 py-2 text-sm font-extrabold transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-sun text-ink shadow-[3px_3px_0px_0px_#fff]' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
        ...
        <a href="{{ route('admin.events.index') }}"
           class="flex items-center gap-3 px-3 py-2 text-sm font-extrabold transition-all {{ request()->routeIs('admin.events.*') ? 'bg-sun text-ink shadow-[3px_3px_0px_0px_#fff]' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
        ...
        <a href="{{ route('admin.participants.index') }}"
           class="flex items-center gap-3 px-3 py-2 text-sm font-extrabold transition-all {{ request()->routeIs('admin.participants.*') ? 'bg-sun text-ink shadow-[3px_3px_0px_0px_#fff]' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
        ...
        <a href="{{ route('admin.registrations.index') }}"
           class="flex items-center gap-3 px-3 py-2 text-sm font-extrabold transition-all {{ request()->routeIs('admin.registrations.*') ? 'bg-sun text-ink shadow-[3px_3px_0px_0px_#fff]' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
