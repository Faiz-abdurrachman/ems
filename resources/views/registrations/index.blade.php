...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-ink/70 font-bold">Kelola pendaftaran peserta ke event</p>
        </div>
    ...
    <div class="mb-6">
        <form method="GET" action="{{ route('admin.registrations.index') }}" class="relative max-w-md">
            ...
            @if(request('search'))
                <a href="{{ route('admin.registrations.index') }}" class="absolute inset-y-0 right-0 flex items-center pr-3 text-ink/60 hover:text-ink/70" aria-label="Hapus pencarian">
    ...
    @if($registrations->isEmpty())
        <div class="flex flex-col items-center justify-center border-2 border-black bg-white py-16">
            ...
            <p class="mt-4 text-sm text-ink/60">Belum ada registrasi</p>
            <a href="{{ route('admin.registrations.create') }}"
            ...
            <table class="min-w-full divide-y-2 divide-black">
                <thead class="bg-gray-50 sticky top-0 z-10">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/70 font-bold">Participant</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/70 font-bold">Event</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/70 font-bold">Registration Date</th>
                ...
                <tbody class="divide-y-2 divide-black">
                    @foreach($registrations as $registration)
                        <tr class="hover:bg-mint/10 transition-colors">
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-ink/60">{{ $loop->iteration + (($registrations->currentPage() - 1) * $registrations->perPage()) }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.registrations.show', $registration) }}"
                                       class="rounded-none p-1.5 text-ink/60 hover:bg-mint/10 hover:text-ink/70 transition-colors"
