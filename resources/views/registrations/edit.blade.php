...
<div class="max-w-2xl mx-auto">
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-200 px-6 py-4">
            <h2 class="text-lg font-semibold text-gray-900">Edit Registration</h2>
            <p class="mt-0.5 text-sm text-gray-500">Update the event and participant for this registration.</p>
        </div>

        <form action="{{ route('registrations.update', $registration) }}" method="POST" class="px-6 py-5 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="participant_id" id="participant_id"
                    ...
                    @foreach($participants as $participant)
                        <option value="{{ $participant->id }}" {{ old('participant_id', $registration->participant_id) == $participant->id ? 'selected' : '' }}>
