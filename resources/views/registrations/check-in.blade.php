    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <div class="h-3 w-40 overflow-hidden rounded-full bg-gray-100">
                <div class="h-full rounded-full bg-mint/200 transition-all" style="width: {{ $total > 0 ? round($attended / $total * 100) : 0 }}%"></div>
            </div>
                        ...
                        @foreach($event->registrations as $reg)
                            <tr class="hover:bg-sun/20 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-6 py-3">
                                    @if($reg->attended_at)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-mint/20 px-2.5 py-0.5 text-xs font-bold text-emerald-700">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-bold text-ink/50 font-bold">
                                ...
                                <td class="whitespace-nowrap px-6 py-3 text-right">
                                    <form method="POST" action="{{ route('admin.registrations.toggle-attendance', $reg) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="rounded-lg px-3 py-1.5 text-xs font-bold {{ $reg->attended_at ? 'bg-coral/20 text-red-600 hover:bg-red-100' : 'bg-mint/20 text-emerald-600 hover:bg-emerald-100' }} transition-colors">
