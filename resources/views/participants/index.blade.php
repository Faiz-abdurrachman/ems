...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-ink/50 font-bold">Kelola data peserta yang terdaftar</p>
        </div>
    ...
    <div class="mb-6">
        <form method="GET" action="{{ route('admin.participants.index') }}" class="relative max-w-md">
            ...
            @if(request('search'))
                <a href="{{ route('admin.participants.index') }}" class="absolute inset-y-0 right-0 flex items-center pr-3 text-ink/40 hover:text-ink/70" aria-label="Hapus pencarian">
    ...
    @if($participants->isEmpty())
        <div class="flex flex-col items-center justify-center border-2 border-black bg-white py-16">
            ...
            <p class="mt-4 text-sm text-ink/40">Belum ada peserta terdaftar</p>
            <a href="{{ route('admin.participants.create') }}"
               class="mt-4 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-extrabold text-white hover:bg-black hover:text-white transition-colors">
            ...
            <table class="min-w-full divide-y-2 divide-black">
                <thead class="bg-gray-50 sticky top-0 z-10">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">Name</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">Email</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">NIM</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">Jurusan</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">Phone</th>
                        <th scope="col" class="px-6 py-3.5 text-left text-xs font-extrabold uppercase tracking-wider text-ink/50 font-bold">Events Joined</th>
                ...
                <tbody class="divide-y-2 divide-black">
                    @foreach($participants as $participant)
                        <tr class="hover:bg-sun/20 transition-colors">
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-ink/40">{{ $loop->iteration + (($participants->currentPage() - 1) * $participants->perPage()) }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm">
                                <span class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-bold text-ink">
